document.addEventListener("DOMContentLoaded", function() {
    document.getElementById("btnBuscar").addEventListener("click", buscarTempo);
});
...
async function buscarTempo() {
    ...
    if (tempo) {
        resultadoDiv.innerHTML = `
                ...
                <p><strong>Visibilidade:</strong> ${tempo.visibility}m</p>
...
async function getPrevisao(cidade) {
    ...
    const url = `https://api.openweathermap.org/data/2.5/weather?q=${cidade}&appid=${chave}&units=metric&lang=pt`;
